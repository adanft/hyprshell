#!/usr/bin/env bash
#
# Builds bagent from source and puts it where the shell can find it.
#
# This is the development path. An installed shell already has bagent: the
# release ships it built, and ../install.sh places it. Reach for this one when
# you are changing bagent itself.
#
#     ./scripts/install-bagent.sh                        into ~/.local/bin
#
#     ./scripts/install-bagent.sh                        for everyone on the
#     PREFIX=/usr/local sudo ./scripts/install-bagent.sh  machine: build as
...
# the shell itself runs from the repository root.
#
# Exits non-zero if the build fails or the binary cannot be placed.

set -uo pipefail

# The repository root, not scripts/ — every path below is relative to it.
