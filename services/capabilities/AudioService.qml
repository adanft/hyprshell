...
Scope {
    ...
    function selectAudioSource(node) {
        if (!node || !root.audioSources.includes(node) || node === root.source)
            return
        Pipewire.preferredDefaultAudioSource = node
    }

    function selectAudioSink(node) {
        if (!AudioNodeState.containsCurrentNode(root.allAudioNodes, node, AudioNodeState.isPhysicalOutput))
            return false
        if (node === root.sink)
            return true
        try {
    ...
    function togglePlaybackStreamMute(node) {
        try {
            if (!root.isWritablePlaybackStream(node))
                return false
            node.audio.muted = !node.audio.muted
            return true
        } catch (_) {
    ...
    function changeVolume(isSource, delta) {
        ...
        if (!node?.audio)
            return
        try {
            if (!isSource && !root.isCurrentPhysicalSink(node))
                return root.failMasterWrite(null)
