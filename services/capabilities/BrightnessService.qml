...
Scope {
    ...
    FileView {
        ...
        watchChanges: true
        printErrors: false
        onLoaded: root.updateBrightnessFromFiles(false)
        onLoadFailed: root.clearBrightnessState()
        onFileChanged: root.updateBrightnessFromFiles(true)
    }
    FileView {
    ...
    FileView {
        ...
        path: root.quickBrightnessPath
        blockLoading: true
        watchChanges: true
        printErrors: false
        atomicWrites: false
        onLoaded: root.refreshQuickBrightness(false)
        onLoadFailed: root.failQuickBrightnessRead("Brightness unavailable")
        onFileChanged: root.refreshQuickBrightness(true)
        onSaved: root.refreshQuickBrightness(true)
        onSaveFailed: root.failQuickBrightnessRequest("write_failed", "Brightness adjustment failed")
    }
    FileView {
        id: quickMaxBrightnessValueFile
        path: root.quickMaxBrightnessPath
        blockLoading: true
        watchChanges: true
        printErrors: false
        onLoaded: root.refreshQuickBrightness(false)
    ...
    Process {
        ...
        stdout: StdioCollector {
            onStreamFinished: root.detectBrightnessDevice(this.text)
        }
    }
    Process {
        id: brightnessWriteProcess
        onExited: root.scheduleBrightnessWrite()
    }
    Timer {
        id: brightnessWriteDebounceTimer
        interval: root.brightnessWriteDebounceMs
        repeat: false
        onTriggered: root.flushBrightnessWrite()
    }
    Timer {
        id: quickBrightnessConfirmationTimer
        interval: 1500
        repeat: false
        onTriggered: root.failQuickBrightnessRequest("reconciliation_timeout", "Brightness adjustment failed")
    }
    ...
    function detectBrightnessDevice(output) {
        ...
        if (match)
            brightnessLevel = Math.max(0, Math.min(100, parseInt(match[1], 10)))
