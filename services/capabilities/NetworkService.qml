...
Scope {
    ...
    onLanInterfaceChanged: {
        ethernetInfoRequestGeneration += 1; ethernetInfoRequestedInterface = ""; ethernetInfo = NetworkState.parseNmcliDeviceInfo("")
        ethernetProfileActionGeneration += 1
        if (ethernetProfileBusy || ethernetProfileAwaitingRefresh) {
            ethernetProfileAwaitingRefresh = false; ethernetProfileBusy = false; ethernetProfilePendingUuid = ""
            if (ethernetProfileError.length === 0) ethernetProfileError = "Ethernet interface changed"
        }
    ...
    FileView { id: networkRxBytes; path: `/sys/class/net/${root.activeNetworkInterface}/statistics/rx_bytes`; blockLoading: true; printErrors: false }
    FileView { id: networkTxBytes; path: `/sys/class/net/${root.activeNetworkInterface}/statistics/tx_bytes`; blockLoading: true; printErrors: false }
    Timer { interval: root.networkRefreshMs; running: root.networkThroughputEnabled; repeat: true; onTriggered: root.refreshNetwork() }
    Timer { interval: 3000; running: root.networkDetailsEnabled && root.lanDevice !== null; repeat: true; triggeredOnStart: true; onTriggered: root.refreshEthernetInfo() }
    Timer { interval: 3000; running: root.networkDetailsEnabled && root.wifiDevice !== null; repeat: true; triggeredOnStart: true; onTriggered: root.refreshWifiInfo() }
    Timer { id: ethernetActionRefreshTimer; interval: 750; repeat: false; onTriggered: root.refreshEthernetInfo() }
    ...
    Process {
        ...
        onExited: (exitCode, exitStatus) => {
            ...
            if (current && root.ethernetInfoProcessRefreshesProfile && root.ethernetProfileAwaitingRefresh) {
                root.ethernetProfileAwaitingRefresh = false; root.ethernetProfileBusy = false; root.ethernetProfilePendingUuid = ""
                if (exitCode !== 0 && root.ethernetProfileError.length === 0) root.ethernetProfileError = `Ethernet refresh failed (${exitCode})`
            }
    ...
    Process {
        ...
        stdout: StdioCollector { onStreamFinished: {
            if (root.wifiInfoProcessGeneration === root.wifiInfoRequestGeneration && root.wifiInfoRequestedInterface === root.wifiInterface) {
                root.wifiInfo = NetworkState.parseNmcliDeviceInfo(this.text); root.wifiInfoAvailability = "available"
            }
        } }
        onExited: (exitCode, exitStatus) => {
            ...
            if (exitCode !== 0 && current) { root.wifiInfo = NetworkState.parseNmcliDeviceInfo(""); root.wifiInfoAvailability = "unavailable" }
            if (!current && root.wifiInterface.length > 0) Qt.callLater(root.refreshWifiInfo)
        }
    }
    Process {
        ...
        onExited: (exitCode, exitStatus) => {
            if (root.ethernetProfileActionProcessGeneration !== root.ethernetProfileActionGeneration) return
            if (exitCode !== 0 && root.ethernetProfileError.length === 0) root.ethernetProfileError = `NetworkManager command failed (${exitCode})`
            root.ethernetProfileAwaitingRefresh = true; ethernetActionRefreshTimer.restart()
        }
    }
    ...
    function enableNetworkDetails() { networkDetailsSubscriberCount++ }
    function disableNetworkDetails() { networkDetailsSubscriberCount = Math.max(0, networkDetailsSubscriberCount - 1) }
    function resetNetworkSample(clearInterface) {
        activeNetworkRxRate = 0; activeNetworkTxRate = 0; previousNetworkRx = 0; previousNetworkTx = 0; previousNetworkSampleMs = 0
        if (clearInterface) previousNetworkInterface = ""
    }
    function refreshEthernetInfo() {
        if (!/^[A-Za-z0-9._:-]{1,64}$/.test(lanInterface)) {
            ...
            if (ethernetProfileAwaitingRefresh) {
                ethernetProfileAwaitingRefresh = false; ethernetProfileBusy = false; ethernetProfilePendingUuid = ""
                if (ethernetProfileError.length === 0) ethernetProfileError = "Ethernet interface unavailable"
            }
            return
        }
        if (ethernetInfoProcess.running) return
        ethernetInfoRequestedInterface = lanInterface; ethernetInfoProcessGeneration = ethernetInfoRequestGeneration
    ...
    function refreshWifiInfo() {
        if (wifiInfoProcess.running) return
        if (!/^[A-Za-z0-9._:-]{1,64}$/.test(wifiInterface)) { wifiInfoRequestedInterface = ""; wifiInfoAvailability = "idle"; wifiInfo = NetworkState.parseNmcliDeviceInfo(""); return }
        if (wifiInfoAvailability !== "available") wifiInfoAvailability = "loading"
        wifiInfoRequestedInterface = wifiInterface; wifiInfoProcessGeneration = wifiInfoRequestGeneration
    ...
    function setEthernetProfileEnabled(profile) {
        ...
        if (ethernetProfileActionProcess.running) return
        const action = NetworkState.ethernetProfileAction(ethernetInfo.activeUuid, uuid, ethernetProfileBusy)
        if (!action) return
        ethernetProfileBusy = true; ethernetProfileAwaitingRefresh = false; ethernetProfilePendingUuid = uuid; ethernetProfileError = ""
    ...
    function refreshNetwork() {
        if (!networkThroughputEnabled || !activeNetworkInterface) { resetNetworkSample(true); return }
        if (previousNetworkInterface !== activeNetworkInterface) { previousNetworkInterface = activeNetworkInterface; resetNetworkSample(false) }
        ...
        if (rxText === null || txText === null) { resetNetworkSample(false); return }
        const now = Date.now(); const rx = Number(rxText.trim()); const tx = Number(txText.trim())
        const elapsed = previousNetworkSampleMs > 0 ? (now - previousNetworkSampleMs) / 1000 : 0
        if (!Number.isFinite(rx) || !Number.isFinite(tx)) { resetNetworkSample(false); return }
        if (previousNetworkRx > 0 && previousNetworkTx > 0 && Number.isFinite(elapsed) && elapsed > 0) {
            activeNetworkRxRate = Math.max(0, (rx - previousNetworkRx) / elapsed); activeNetworkTxRate = Math.max(0, (tx - previousNetworkTx) / elapsed)
        }
        previousNetworkRx = rx; previousNetworkTx = tx; previousNetworkSampleMs = now
    }
}
