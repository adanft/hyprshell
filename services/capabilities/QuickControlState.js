...
function validBrightnessDevicePath(value) {
	return (
		typeof value === "string" &&
		value.length <= 4096 &&
		/^\/sys\/class\/backlight\/[A-Za-z0-9][A-Za-z0-9._-]{0,127}$/.test(
			value,
		)
	);
}
...
function normalizedReadback(currentText, maximumText) {
	...
	if (current === null || maximum === null || current > maximum) return null;
	return {
		rawCurrent: current,
		rawMaximum: maximum,
...
function rawForPercent(percent, maximum) {
	...
	return Math.max(
		0,
		Math.min(maximum, Math.round((normalized * maximum) / 100)),
	);
}

function normalizedVolumeRequest(percent, available) {
	var normalized = clampPercent(percent);
	if (!available || normalized === null) return null;
	return {
		percent: normalized,
...
function syncConfirmed(capability, percent) {
	...
	if (
		capability.availability !== "interacting" &&
		capability.availability !== "pending_confirmation"
	)
...
function beginInteraction(capability, percent) {
	var normalized = clampPercent(percent);
	if (normalized === null) return { state: capability, effect: null };
	return {
...
function failRequest(capability, requestId, errorCode, errorText) {
	if (capability.activeRequestId !== requestId) return capability;
