...
for (const candidate of [
	...
	assert.equal(
		state.validBrightnessDevicePath(candidate),
		false,
		String(candidate),
	);
...
assert.deepEqual(
	JSON.parse(JSON.stringify(state.normalizedReadback("40\n", "160\n"))),
	{
		rawCurrent: 40,
		rawMaximum: 160,
		percent: 25,
...
assert.deepEqual(
	JSON.parse(JSON.stringify(state.normalizedVolumeRequest(42.6, true))),
	{
		percent: 43,
		volume: 0.43,
		unmute: true,
...
assert.deepEqual(
	JSON.parse(JSON.stringify(state.normalizedVolumeRequest(0, true))),
	{
		percent: 0,
		volume: 0,
		unmute: false,
...
assert.equal(
	capability.authoritativePercent,
	null,
	"unavailable must not fabricate 0%",
);
assert.equal(
	state.syncConfirmed(capability, 64).effect,
	null,
	"native sync never writes",
);
...
assert.deepEqual(JSON.parse(JSON.stringify(request.effect)), {
	type: "request",
	percent: 72,
	requestId: 1,
});
...
for (const reason of [
	...
	const failed = state.failRequest(
		request.state,
		1,
		reason,
		"Adjustment failed",
...
const noKnownValue = state.failRequest(
	state.completeInteraction(
		state.beginInteraction(state.unavailableCapability(""), 20).state,
		2,
	).state,
	2,
	"write_failed",
...
console.log(
	"QuickControlState: validation, normalization, gesture, stale, and failure contracts passed",
);
