...
Scope {
    ...
    FileView { id: cpuStatFile; path: "/proc/stat"; blockLoading: true; printErrors: false }
    FileView { id: memoryInfoFile; path: "/proc/meminfo"; blockLoading: true; printErrors: false }
    Timer {
    ...
    function enableCpuUsage() { cpuUsageSubscriberCount++; refreshSystemStats() }
    function disableCpuUsage() {
        cpuUsageSubscriberCount = Math.max(0, cpuUsageSubscriberCount - 1)
        if (!cpuUsageEnabled)
            previousCpuStats = null
    }
    function enableMemoryUsage() { memoryUsageSubscriberCount++; refreshSystemStats() }
    function disableMemoryUsage() { memoryUsageSubscriberCount = Math.max(0, memoryUsageSubscriberCount - 1) }
    function refreshSystemStats() {
        if (cpuUsageEnabled) {
            const text = FileViewState.safeText(cpuStatFile, "CPU stats", true)
            if (text === null) { previousCpuStats = null; cpuUsage = 0 } else updateCpuUsage(text)
        }
        if (memoryUsageEnabled) {
            const text = FileViewState.safeText(memoryInfoFile, "memory stats", true)
            if (text === null) memoryUsage = 0; else updateMemoryUsage(text)
        }
    }
    function updateCpuUsage(text) {
        const line = String(text || "").split("\n")[0]
        if (!line.startsWith("cpu ")) { previousCpuStats = null; cpuUsage = 0; return }
    ...
    function updateMemoryUsage(text) {
        ...
        for (const line of lines) {
            if (line.startsWith("MemTotal:")) total = Number(line.trim().split(/\s+/)[1]) || 0
            else if (line.startsWith("MemAvailable:")) available = Number(line.trim().split(/\s+/)[1]) || 0
        }
