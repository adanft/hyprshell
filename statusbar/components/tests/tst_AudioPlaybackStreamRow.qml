...
TestCase {
    ...
    property var node: ({ nickname: "Music", audio: { volume: 0.42, muted: false } })
    property var colors: ({
        primary: "#88aaff",
        surface: "#202020",
        surfaceHover: "#303030",
    ...
    SignalSpy { id: muteSpy; signalName: "muteRequested" }
    ...
    function test_unavailableIsSafe() {
        const value = row({ available: false });
        value.requestMute();
    ...
    function test_draftsAreLocal() {
        const first = row();
        const second = row({ stream: ({ audio: { volume: 0.8, muted: false } }) });
    ...
    function test_volumeIconIsMuteActionAndPercentStaysReadOnly() {
        ...
        value = row({ stream: ({ nickname: "Music", audio: { volume: 0.42, muted: true } }) });
        compare(findChild(value, "playbackStreamVolumeIcon").text, "M");
        compare(findChild(value, "playbackStreamPercentLabel").text, "42%");
        value = row({ stream: ({ nickname: "Music", audio: { volume: 0, muted: false } }) });
        compare(findChild(value, "playbackStreamVolumeIcon").text, "M");
        value = row({ stream: ({ nickname: "Music", audio: { volume: 0.2, muted: false } }) });
        compare(findChild(value, "playbackStreamVolumeIcon").text, "L");
        value = row({ stream: ({ nickname: "Music", audio: { volume: 0.9, muted: false } }) });
        compare(findChild(value, "playbackStreamVolumeIcon").text, "H");
        value = row({ stream: ({ nickname: "Music", audio: null }) });
