...
TestCase {
    ...
    Component {
        ...
        QtObject {
            ...
            function enableNetworkDetails() { detailEnables += 1; }
            function disableNetworkDetails() { detailDisables += 1; }
        }
    }
    Component {
        id: deviceComponent
    ...
    Component {
        ...
        QtObject {
            ...
            function connect() { connects += 1; }
            function disconnect() { disconnects += 1; }
            function connectWithPsk(password) { pskConnects += 1; lastPassword = password; }
            function forget() { forgets += 1; }
        }
    }
    Component { id: controllerComponent; NetworkMenuController {} }
    ...
    function fixture(values) {
        ...
        const result = { controller, networking, service, device, uptime, devices: [device], networks: [] };
    ...
    function test_uptimeIsOpenOnlyAndRetainsInvalidValue() {
        const f = fixture({});
        compare(f.controller.uptimeRefreshIntervalMs, 60000); compare(f.controller.uptimeRefreshTimer.repeat, true);
        compare(f.controller.uptimeRefreshTimer.running, false); compare(f.uptime.reloads, 1);
        compare(f.controller.uptimeSeconds, 42.5);
        f.uptime.value = "invalid"; f.controller.refreshUptime();
        compare(f.controller.uptimeSeconds, 42.5);
        f.controller.prepareOpen(); compare(f.uptime.reloads, 3);
        f.controller.menuOpen = true; compare(f.controller.uptimeRefreshTimer.running, true);
        f.controller.menuOpen = false; compare(f.controller.uptimeRefreshTimer.running, false);
    }

    function test_timerMetadataAndActivationGeneration() {
        const f = fixture({ networking: { wifiEnabled: false } });
    ...
    function test_disabledAndClosedCallbacksDoNothing() {
        ...
        const activation = fixture({ networking: { wifiEnabled: false } });
    ...
    function test_replacementReleasesOnlyOwnedScanner() {
        ...
        const foreign = fixture({ device: { scannerEnabled: true } });
    ...
    function test_connectionCommandsDispatchExactlyOnce() {
        const f = fixture({});
        const connected = network(f, { connected: true });
        const known = network(f, { known: true });
        const open = network(f, { security: 0 });
        const secured = network(f, { name: "Secured" });
        const saved = network(f, { known: true });
        ...
        const invalid = network(f, { known: false });
        const busyForget = network(f, { known: true, stateChanging: true });
        ...
        f.service.lanDevice = { network: saved };
    ...
    function test_cancelAndClosePreserveSuppressionOrder() {
        const f = fixture({});
        const target = network(f, { name: "Same" });
    ...
    function test_failuresSuccessAndStrictIdentity() {
        const f = fixture({});
        const pending = network(f, { name: "Same" });
        const sameName = network(f, { name: "Same" });
        ...
        compare(f.controller.suppressedPasswordNetwork, null);
        f.service.lanDevice = { network: sameName };
